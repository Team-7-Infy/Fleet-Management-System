-- Full demo dataset for Fleet Manager screens.
-- Run in Supabase SQL Editor.
-- This deletes the old placeholder demo records and replaces the demo UUID range.
-- If public.users.userid references auth.users.id, create matching auth users first
-- or replace these UUIDs with existing auth user IDs.
...
create temporary table seed_users(userid uuid) on commit drop;
insert into seed_users(userid) values
    ...
    ('11000000-0000-0000-0000-000000000106'),
    ('11000000-0000-0000-0000-000000000107'),
    ('11000000-0000-0000-0000-000000000108'),
    ('11000000-0000-0000-0000-000000000109'),
    ('11000000-0000-0000-0000-000000000110'),
    ('11000000-0000-0000-0000-000000000111'),
    ('11000000-0000-0000-0000-000000000112'),
    ...
    ('11000000-0000-0000-0000-000000000206'),
    ('11000000-0000-0000-0000-000000000207'),
    ('11000000-0000-0000-0000-000000000208'),
    ('11000000-0000-0000-0000-000000000209'),
    ('11000000-0000-0000-0000-000000000210'),
    ('11000000-0000-0000-0000-000000000211'),
    ('11000000-0000-0000-0000-000000000212');

create temporary table seed_drivers(driverid uuid) on commit drop;
insert into seed_drivers(driverid) values
...
create temporary table seed_personnel(personnelid uuid) on commit drop;
insert into seed_personnel(personnelid) values
    ...
    ('41000000-0000-0000-0000-000000000211'),
    ('41000000-0000-0000-0000-000000000212');

create temporary table seed_vehicles(vin uuid) on commit drop;
insert into seed_vehicles(vin) values
...
create temporary table seed_trips(tripid uuid) on commit drop;
insert into seed_trips(tripid) values
...
create temporary table seed_tasks(taskid uuid) on commit drop;
insert into seed_tasks(taskid) values
...
create temporary table seed_parts(partid uuid) on commit drop;
insert into seed_parts(partid) values
...
create temporary table bad_users on commit drop as
select userid
from public.users
where userid in (select userid from seed_users)
...
create temporary table bad_drivers on commit drop as
select driverid
from public.drivers
where driverid in (select driverid from seed_drivers)
   or userid in (select userid from bad_users);

create temporary table bad_personnel on commit drop as
select personnelid
from public.maintenance_personnel
where personnelid in (select personnelid from seed_personnel)
   or userid in (select userid from bad_users);

create temporary table bad_managers on commit drop as
select managerid
from public.fleet_manager
where managerid = '21000000-0000-0000-0000-000000000001'
   or userid in (select userid from bad_users);

create temporary table bad_vehicles on commit drop as
select vin
from public.vehicles
where vin in (select vin from seed_vehicles)
   or vin::text like '50000000-%'
   or driverid in (select driverid from bad_drivers)
   or regexp_replace(licence_plate, '[[:space:]]+', '', 'g') in ('UK071234', 'UK07AJ9125', 'UL043456')
...
create temporary table bad_tasks on commit drop as
select taskid
from public.maintenance_task
where taskid in (select taskid from seed_tasks)
   ...
   or executedby in (select personnelid from bad_personnel)
   or scheduledby in (select managerid from bad_managers)
   or taskid in (
        select taskid
        from public.task_vehicles
        where vin in (select vin from bad_vehicles)
   );

create temporary table bad_trips on commit drop as
select tripid
from public.trips
where tripid in (select tripid from seed_trips)
   or tripid::text like '60000000-%'
   or vehicleid in (select vin from bad_vehicles)
   or driverid in (select driverid from bad_drivers)
   or lower(startlocation) in ('dun', 'blr')
   or lower(endlocation) in ('dun', 'blr');

delete from public.maintenance_task_parts
where taskid in (select taskid from bad_tasks)
   or partid in (select partid from seed_parts);

delete from public.task_vehicles
where taskid in (select taskid from bad_tasks)
   or vin in (select vin from bad_vehicles);

delete from public.maintenance_task
where taskid in (select taskid from bad_tasks);

delete from public.route_waypoints
where tripid in (select tripid from bad_trips);
...
where driverid in (select driverid from bad_drivers);
...
where partid in (select partid from seed_parts)
...
where personnelid in (select personnelid from bad_personnel);

delete from public.fleet_manager
where managerid in (select managerid from bad_managers);

delete from public.users
where userid in (select userid from bad_users);
